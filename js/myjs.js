(function (window) {
    ...
    // 进行验证
      verify () {
        ...
        const average = arr.reduce(sum) / arr.length    //计算平均数
        const deviations = arr.map(x => x - average)    //计算偏差
        const stddev = Math.sqrt(deviations.map(square).reduce(sum) / arr.length)   //拖动轨迹
        const left = parseInt(this.block.style.left)    //计算滑片左边的距离
        return {
            // 如果偏差小于10
